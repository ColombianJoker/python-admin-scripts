...
def ConvertImage(
    Background, NewWidth, NewHeight, ResizeWidth, ResizeHeight, InputFile, OutputFile
):
...
def ProcessImage(ImageName):
    ...
    if (ImageProp != Opts.SProp) or Opts.Force:
        ...
        Opts.Log.debug(
            "ImageProp=%0.3f SProp=%0.3f W=%d H=%d"
            % (
                ImageProp,
                Opts.SProp,
                TargetWidth,
...
try:
    ...
    parser.add_option(
        ...
        action="store",
        type="string",
        help="Target directory to copy to",
    ...
    parser.add_option(
        ...
        type="string",
        default="JPG",
        help="Selects image type/format",
    )
    parser.add_option(
    ...
    if Opts.ImageFormat == "JPG":
        ...
        logging.error(
            "%s: Unknown image format specification (%s)!\n"
