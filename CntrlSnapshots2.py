...
def SetLogging(Opts):
    if Opts.DoLog:
        ...
        LogHandler = logging.handlers.TimedRotatingFileHandler(
            Opts.LogFile, "Midnight", 1, backupCount=30
        )
    ...
    try:
        for i in range(len(Opts.CloneTimes)):
            ...
            if (len(CleanedTime) > 0) and (CleanedTime not in Opts.ExecutionTimes):
                ...
                Opts.Log.warning(
                    'could not interpret time "%s", skipping...' % (Opts.CloneTimes[i],)
                )
...
def GetVolumeSize(VolumeName):
    ...
    if Opts.Storage1Type == "storwize":
        ...
        Opts.Log.error(
            "don't know how to get volume size in storage type %s"
            % (Opts.Storage1Type,)
        )
...
def CreateNowNamedVolume(VolumeName):
    ...
    if Opts.Storage1Type == "storwize":
        ...
        Opts.Log.error(
            "don't know how to create volumes in storage type %s" % (Opts.Storage1Type,)
        )
        return COULD_NOT_CREATE
...
def CreateNowNamedCG():
    ...
    DebugFn()
    Opts.Log.info(
        'trying to create new consistency group to snap "%s"...'
        % (Opts.ConsistencyGroupPrefix,)
    )
    if Opts.Storage1Type == "storwize":
        try:
            TimeSuffix = GetTimedSuffix()
        ...
        Opts.Log.error(
            "don't know how to create consistency groups in storage type %s"
            % (Opts.Storage1Type,)
        )
        return COULD_NOT_CREATE
...
def GetListOfNamedSnaps(VolumeName):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            ...
            if len(OutputText) > 0:
                ...
                if len(VolumeList) > 0:
                    if len(VolumeList) == 1:
                        Opts.Log.info(
                            "snapshot of %s only is %s" % (VolumeName, VolumeList)
                        )
                    else:
                        Opts.Log.info(
                            "snapshots of %s are %s"
                            % (
                                VolumeName,
                                VolumeList,
                            )
        ...
        Opts.Log.error(
            "don't know how to get the list of snapshots in storage type %s"
            % (Opts.Storage1Type,)
        )
        return []
...
def CountNamedSnaps(VolumeName):
    ...
    if Opts.DEBUG:
        Opts.Log.debug(
            "%d snapshots found for %s"
            % (len(GetListOfNamedSnaps(VolumeName)), VolumeName)
        )
...
def GetNameOfFirstNamedSnap(VolumeName):
    ...
    if Opts.DEBUG:
        Opts.Log.debug(
            'first snapshot of "%s" is "%s"'
            % (VolumeName, sorted(GetListOfNamedSnaps(VolumeName))[0])
        )
...
def RemoveVolume(VolumeName):
    ...
    if Opts.Storage1Type == "storwize":
        ...
        Opts.Log.error(
            "don't know how to remove volumes in storage type %s" % (Opts.Storage1Type,)
        )
    return
...
def IsVolumeMapped(VolName):
    ...
    DebugFn()
    Opts.Log.debug(
        'trying to check if volume "%s" is mapped to server "%s"'
        % (VolName, Opts.MappedHost)
    )
    isMapped = False
    if Opts.Storage1Type == "storwize":
        try:
                ...
                for TextLine in OutputText.splitlines():
                    if len(TextLine) > 0 and len(TextLine.split()) > 0:
                        if TextLine.split()[4] == Opts.MappedHost:
                            Opts.Log.debug(
                                'volume "%s" is mapped to server "%s"'
                                % (VolName, Opts.MappedHost)
                            )
                            isMapped = True
            return isMapped
        ...
        Opts.Log.error(
            "don't know how check mappings in storage type %s" % (Opts.Storage1Type,)
        )
        return isMapped
...
def UnmapVolume(VolName):
    ...
    DebugFn()
    Opts.Log.debug(
        'trying to unmap volume "%s" from server "%s"' % (VolName, Opts.MappedHost)
    )
    if Opts.Storage1Type == "storwize":
        try:
            CxCommand = "rmvdiskhostmap %s %s" % (
                Opts.MappedHost,
                VolName,
            )
            OutputText = UNIXconnect(Opts.Storage1User, Opts.Storage1Name, CxCommand)
            Opts.Log.info(
                'volume "%s" unmapped from server "%s"' % (VolName, Opts.MappedHost)
            )
        except:
            Opts.Log.warning(
                'volume "%s" could not be unmapped from server "%s"'
                % (VolName, Opts.MappedHost)
            )
            return False
    else:
        Opts.Log.error(
            "don't know how to unmap in storage type %s" % (Opts.Storage1Type,)
        )
        return False
...
def SnapVolumeToVolume(SourceVolume, TargetVolume, ConsistencyGroupName):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            ...
            CxCommand = (
                "mkfcmap -source %s -target %s -name %s -copyrate %d -consistgrp %s -autodelete"
                % (
                    SourceVolume,
                    TargetVolume,
                    SnapshotNow,
            ...
            OutputText = UNIXconnect(Opts.Storage1User, Opts.Storage1Name, CxCommand)
            Opts.Log.info(
                'snapshot of volume "%s" created into "%s" (pair named "%s")'
                % (SourceVolume, TargetVolume, SnapshotNow)
            )
            return True
        except:
            Opts.Log.error(
                'snapshot of volume "%s" could not created into "%s"!'
...
def StartSyncSnaps(ConsistencyGroupName):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            ...
            OutputText = UNIXconnect(Opts.Storage1User, Opts.Storage1Name, CxCommand)
            Opts.Log.info(
                'snapshots of consistency group "%s" started' % (ConsistencyGroupName,)
            )
            return True
        except:
            Opts.Log.error(
                'snapshots of consistency group "%s" failed' % (ConsistencyGroupName,)
            )
    else:
        Opts.Log.error(
            "don't know how to get the list of snapshots in storage type %s"
...
def MapOneVolume(VolName, ScsiId):
    ...
    DebugFn()
    Opts.Log.debug(
        'trying to map volume "%s"->%d to server "%s"'
        % (VolName, ScsiId, Opts.MappedHost)
    )
    if Opts.Storage1Type == "storwize":
        try:
            CxCommand = "mkvdiskhostmap -host %s -scsi %d -force %s" % (
                Opts.MappedHost,
                ScsiId,
                VolName,
            )
        ...
        Opts.Log.error(
            "don't know how to map volumes in storage type %s" % (Opts.Storage1Type,)
        )
        return False
    return True
...
def GetLastScsiId(MappedHost):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            ...
            if LastScsiFound != -1:
                Opts.Log.info(
                    'Last SCSI LUN found in %s was #%d "%s"'
                    % (MappedHost, LastScsiFound, LastVolFound)
                )
            else:
        ...
        Opts.Log.error(
            "don't know how to get list of mapped volumes in storage type %s"
            % (Opts.Storage1Type,)
        )
        return LastScsiFound
    return 0
...
def MapVolumes(VolumeList):
    ...
    DebugFn()
    Opts.Log.debug(
        'trying to map volumes %s to server "%s"' % (VolumeList, Opts.MappedHost)
    )
    if Opts.Storage1Type == "storwize":
...
def ExitIfKillFile():
    ...
    if os.path.exists(Opts.KillFile):
        ...
        try:
            ...
            Opts.Log.info(
                "%s: kill file found and removed, exiting..." % (Opts.PrgName,)
            )
        except IOError:
            Opts.Log.info(
...
def TryOneScript(CommandString):
    ...
    try:
        ...
        Opts.Log.error(
            "TryOneScript() could not execute CommandString=%s" % (CommandString,)
        )
        if not Opts.DEBUG:
            Opts.ExitCode = 8
            sys.exit(
...
def WaitNextIteration():
    ...
    Opts.Log.debug(
        "waiting for next iteration, now is %s seq minutes" % (StartingMinute,)
    )
    while True:
...
try:
    ...
    parser.add_option(
        "--cg",
        "--consistency-group",
        dest="ConsistencyGroupPrefix",
        action="store",
        type="string",
        help="Consistency group name prefix",
        default="SAP_Replica_",
    )
    parser.add_option(
        "--ps",
        "--pre-script",
    ...
    parser.add_option(
        ...
        action="store",
        type="string",
        help="Post- script to execute to",
        default="/usr/bin/true",
    )
    parser.add_option(
        "--Ps2",
    ...
    parser.add_option(
        "--sb",
        "--script-before-cloning",
        dest="ScriptBeforeCloning",
        action="store_true",
        help="Run script before cloning",
        default=False,
    )
    parser.add_option(
        "-H",
        ...
        action="store",
        type="string",
        help="Name of host (on storage) using the volumes",
        default=MappedHost,
    )
    parser.add_option(
        "-n",
        ...
        action="store",
        type="int",
        help="How many snapshots to maintain",
        default=6,
    )
    parser.add_option(
        "-p",
    ...
    logging.error(
        "%s: IO Error changing directory to %s!" % (Opts.PrgName, Opts.WorkDir)
    )
    sys.stderr.write(
