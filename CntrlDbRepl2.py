...
def RemoveCloneOnTarget(Opts, Num):
    ...
    Msg(
        Opts,
        "trying to remove disk %s (%s) from VM #%s..."
        % (Num, Opts.CloneDisk[Num], Opts.VMid),
    )
    Opts.Raise = False
    if Opts.VHostType == "vmware":
...
def UnmapCloneLun(Opts, VolumeName):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            ...
            CxCommand = "rmvdiskhostmap -host %s %s" % (
                "SR-" + Opts.VHost1Name,
                VolumeName,
            )
            OutputText = UNIXconnect(
        ...
        try:
            ...
            if len(OutputLines) > 0 and len(OutputLines[0]) > 0:
                for OneLine in OutputLines:
                    ...
                    if (
                        len(OutputFields[2]) > 0
                        and int(OutputFields[2]) > Opts.SCSIoffset
                    ):
                        ...
                        try:
                            ...
                            ErrMsg(
                                Opts,
                                "could not unmap %s [%d] from %s!"
                                % (LUname, LUnumber, ESXHost),
                            )
            else:
                ErrMsg(
                    Opts,
                    "could not understand output of lshostvdiskmap %s [got %s]!"
                    % (ESXHost, OutputText),
                )
        except:
    ...
    if Opts.VHostType == "vmware":
        try:
            ...
            if len(Opts.SnapshotUUID[Num]) < 35:
                ErrMsg(
                    Opts,
                    "%s could not get %s snapshot UUID! (got %s)\n%s: Exiting..."
                    % (
                        FnName,
                        Opts.CloneDisk[Num],
            ...
            if len(Opts.SnapshotUUID[Num]) > 35:
                ErrMsg(
                    Opts,
                    "%s got too much for %s snapshot UUID! (got %s)\n%s: Exiting..."
                    % (
                        FnName,
                        Opts.CloneDisk[Num],
                        Opts.SnapshotUUID[Num],
            ...
            Msg(
                Opts,
                "got UUID %s for %s" % (Opts.SnapshotUUID[Num], Opts.CloneDisk[Num]),
            )
        except:
    ...
    if Opts.VHostType == "vmware":
        try:
            ...
            for OneLUN in Opts.LUN:
                CxCommand = (
                    "esxcli storage filesystem list | awk '$2~/snap.%s/{print $1} | tee SNAPSHOT%sFS.txt"
                    % (OneLUN, OneLUN)
                )
                OutputText = UNIXconnect(
...
try:
    ...
    parser.add_option(
        "-T", "--clone-time", dest="CloneTimes", action="append", type="string"
    )
    parser.add_option(
        "-K",
        "--kill",
