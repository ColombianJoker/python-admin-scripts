    ...
    if os.path.getsize(FileName):
        ...
        if Opts.DeleteAttr:
            ...
            if StoredFileHash == "#ERROR#":
                if Opts.Verbose:
                    sys.stdout.write(
                        '%s: file "%s" has no stored %s attribute. Could not compare.\n'
                        % (Opts.PrgName, FileName, Opts.HashName)
                    )
                    sys.stdout.flush()
                else:
...
parser.add_option(
    ...
    dest="StoreAttr",
    action="store_true",
    help="Store extended attributes if not found",
    default=False,
)
parser.add_option(
    "-y",
    "--always-recreate-xattr",
    dest="ForceCreateAttr",
    action="store_true",
    help="Always recreate extended attributes",
    default=False,
)
parser.add_option(
    "-d",
    ...
    dest="DeleteAttr",
    action="store_true",
    help="Delete stored extended attribute if found",
    default=False,
)
parser.add_option(
    "-c",
    ...
    dest="VerifyAttr",
    action="store_true",
    help="Check file against stored extended attribute",
    default=False,
)
parser.add_option(
    "-m",
    ...
    action="store",
    type="string",
    help="Digest algoritm type (SHA1, SHA224, SHA256, SHA384, SHA512, MD2, MD4, MD5, MDC2, RMD160, defaults to MD5)",
    default="md5",
    metavar="MODE",
)
parser.add_option(
...
if len(Args) > 0:
    if Options.Verbose:
        ...
        if Options.ForceCreateAttr:
            print(
                "%s creating and unconditionally recreating extented attributes (%s)"
                % (Options.PrgName, Options.HashName)
            )
        elif Options.StoreAttr:
            print(
                "%s creating and saving extented attributes (%s)"
                % (Options.PrgName, Options.HashName)
            )
        elif Options.DeleteAttr:
            print(
                "%s deleting extented attributes (%s)"
                % (Options.PrgName, Options.HashName)
            )
    for OneArg in Args:
