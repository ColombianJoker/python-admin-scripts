...
def RemoveVolume(Opts, VolumeName):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            ...
            sys.stderr.write(
                "%s: volume %s could not be removed.\n" % (Opts.PrgName, VolumeName)
            )
    else:
        sys.stderr.write(
            "%s: volume %s could not be removed.\n" % (Opts.PrgName, VolumeName)
...
def SnapVolumeToVolume(Opts, SourceVolume, TargetVolume):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            ...
            Msg(
                Opts,
                "%s: snapshot of volume %s created into %s (pair named %s)"
                % (Opts.PrgName, SourceVolume, TargetVolume, SnapshotNow),
            )
        except:
            sys.stderr.write(
                "%s: snapshot of volume %s could not created into %s!\n"
