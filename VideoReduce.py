...
def extractVideo(Opts, File):
    ...
    if (
        os.path.isfile(Opts.TempOutputVideo)
        and os.path.getsize(Opts.TempOutputVideo) > 0
    ):
        returnCode = True
    else:
        returnCode = False
...
def extractAudio(Opts, File):
    ...
    if Opts.Debug:
        for lineItem in suppressOutput:
            sys.stderr.write(lineItem + "\n")
    if (
        os.path.isfile(Opts.TempOutputAudio)
        and os.path.getsize(Opts.TempOutputAudio) > 0
    ):
        returnCode = True
    else:
        returnCode = False
...
def mediaCombine(Opts, File):
    ...
    if len(getDirName(File)) > 0:
        Opts.FinalVideo = (
            getDirName(File) + "/" + getBaseName(File) + Opts.OutputExtension
        )
    else:
    ...
    if (
        os.path.isfile(Opts.TempOutputVideo)
        and os.path.getsize(Opts.TempOutputVideo) > 0
        and os.path.isfile(Opts.TempOutputAudio)
        and os.path.getsize(Opts.TempOutputAudio) > 0
    ):
        CommandLine = (
            Opts.ffMpeg
            + " "
            + Opts.ffMpegFmt
            % (Opts.TempOutputVideo, Opts.TempOutputAudio, Opts.FinalVideo)
        )
        if Opts.Debug:
            sys.stderr.write("%s\n" % (CommandLine,))
        p = os.popen(CommandLine, "r")
...
def processOneMediaFile(Opts, File):
    ...
    if not extractVideo(Opts, File):
        ...
        if not extractAudio(Opts, File):
            sys.stderr.write(
                "%s: Could not get audio from '%s'!\n" % (Opts.PrgName, File)
            )
            returnCode = False
        else:
            if Opts.Verbose:
                sys.stdout.write(
                    "%s: Audio extracted from '%s'.\n" % (Opts.PrgName, File)
                )

            # Combine/render media file
            if not mediaCombine(Opts, File):
                sys.stderr.write(
                    '%s: Could not recombine audio and video into "%s"!\n'
                ...
                if Opts.Verbose:
                    sys.stdout.write(
                        '%s: Media file recombined into "%s".\n'
                        % (Opts.PrgName, Opts.FinalVideo)
                    )
    return returnCode
...
def main():
    ...
    parser.add_option(
        ...
        "--ext",
        "--extension",
        dest="OutputExtension",
        action="store",
        default=".avi",
        help="Default extension of created video files",
    )
    parser.add_option(
        "-n",
        "--nokia",
        ...
        action="store_true",
        default=False,
        help="Create a small Nokia sized video",
    )
    ...
    if Options.NokiaSize:
        ...
        Options.mPlayerFmt = (
            '-vo null -ao pcm:file="%s" -af resample=8000,volume=+4db:sc "%s"'
        )
        ...
        # First input file, Second output file
        Options.mPlayerFmt = (
            '-vo null -ao pcm:file="%s" -af resample=16000,volume=+4db:sc "%s"'
        )
        # First output file, Second input file
        Options.ffMpegFmt = '-i "%s" -i "%s" -b 48 -ac 1 -ab 12 -map 0.0 -map 1.0 "%s"'
        # First video input, Second audio input, Third output
