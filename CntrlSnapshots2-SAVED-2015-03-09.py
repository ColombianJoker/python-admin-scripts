...
Last mod by Ramón Barrios Láscar on 2015/03/06
...
def CheckCloneTimes():
  DebugFn()
  Opts.Log.debug("validating clone times given like arguments")
  try:
    for i in range(len(Opts.CloneTimes)):
      ...
      if (len(CleanedTime)>0) and (CleanedTime not in Opts.ExecutionTimes):
        ...
        Opts.Log.warning("could not interpret time \"%s\", skipping...\n"%(Opts.CloneTimes[i],))
...
def MapOneVolume(VolName,ScsiId):
  ...
  if Opts.Storage1Type=="storwize":
    try:
      CxCommand="mkvdiskhostmap -host %s -scsi %d %s"%(Opts.MappedHost,ScsiId,VolName)
      OutputText=UNIXconnect(Opts.Storage1User,Opts.Storage1Name,CxCommand)
...
def GetLastScsiId(MappedHost):
  ...
  if Opts.Storage1Type=="storwize":
    try:
      ...
      if len(OutputText)>0 and len(OutputText.splitlines())>0:
        for TextLine in OutputText.splitlines():
          if len(TextLine) and len(TextLine.split()[2]) and isinstance(TextLine.split()[2],int):
            if int(TextLine.split()[2])>LastScsiFound:
              LastScsiFound=int(TextLine.split()[2])
      Opts.Log.debug("Last SCSI LUN found #%d \"%s\""%(TextLine.split()[2],TextLine.split()[4]))
        ...
        while CountNamedSnaps(OneVolume)>Opts.Snapshots:
          ...
          try:
            ...
            if IsVolumeMapped(FirstSnapshot):
              ...
              VolumesToMap.append(FirstSnapshot)
