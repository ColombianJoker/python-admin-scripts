...
def UNIXconnect(Opts, UserName, ServerName, CommandString):
    ...
    try:
        ...
        if not Opts.DEBUG:
            ...
            sys.exit(
                Opts.ExitCode
            )  # Exit code 8 now defined as ERROR in SSH CONNECTION
    return CxLines
...
def GetVolumeSize(Opts, VolumeName):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            ...
            OutputText = UNIXconnect(
                Opts, Opts.Storage1User, Opts.Storage1Name, CxCommand
            )
            if len(OutputText) > 0:
                # Filter lines
...
def GetVMID(Opts, VMname):
    ...
    if Opts.VHost1Type == "vmware":
        CxCommand = (
            "vim-cmd vmsvc/getallvms | awk '$2~/%s$/ {print $1}' | tee %sVM.txt"
            % (
                Opts.VMre,
                Opts.Server3VMname,
            )
        ...
        try:
            ...
            if len(Opts.VMid) == 0:
                ErrMsg(
                    Opts,
                    "could not get the VM ID of %s, exiting..." % (Opts.Server3Name,),
                )
                Opts.ExitCode = 50
            ...
            try:
                OutputText = UNIXconnect(
                    Opts, Opts.VHost2User, Opts.VHost2Name, CxCommand
                )
                Opts.VMid = OutputText.splitlines()[0]
                if len(Opts.VMid) == 0:
                    ErrMsg(
                        Opts,
                        "could not get the VM ID of %s, exiting..."
                        % (Opts.Server3Name,),
                    )
                    Opts.ExitCode = 50
                    sys.exit(Opts.ExitCode)
        ...
        ErrMsg(
            Opts,
            "don't know how to shutdown %s (%s) on %s!"
            % (Opts.Server3Name, Opts.Server3VMname, Opts.VHost1Name),
        )
        Opts.ExitCode = 10
        sys.exit(Opts.ExitCode)
...
def GetTargetVMState(Opts):
    ...
    if Opts.VHostType == "vmware":
        CxCommand = (
            "vim-cmd vmsvc/power.getstate %s | tail -1 | sed 's/power/Power/g;s/on/On/;s/off/Off/;s/ //g'"
            % (Opts.VMid)
        )
        try:
            ...
            if len(Opts.VMid) == 0:
                ErrMsg(
                    Opts,
                    "could not get the state of VM %s (%s) on %s, exiting..."
                    % (Opts.Server3Name, Opts.VMid, Opts.VHostName),
                )
                Opts.ExitCode = 51
                sys.exit(Opts.ExitCode)
...
def ShutdownTargetVM(Opts):
    ...
    if Opts.VHostType == "vmware":
        if Opts.VMstate != "PoweredOff":
            CxCommand = (
                "vim-cmd vmsvc/power.shutdown %s 2>&1| awk '/existingState/{print substr($3,2,length($3)-3)}' "
                % (Opts.VMid,)
            )
            OutputText = UNIXconnect(Opts, Opts.VHostUser, Opts.VHostName, CxCommand)
        ...
        ErrMsg(
            Opts,
            "don't know how to shutdown %s (%s) on %s!"
            % (Opts.Server3Name, Opts.Server3VMname, Opts.VHostName),
        )
        Opts.ExitCode = 11
        sys.exit(Opts.ExitCode)
    return True
...
def PowerOffTargetVM(Opts):
    ...
    if Opts.VHostType == "vmware":
        ...
        ErrMsg(
            Opts,
            "don't know how to stop %s (%s) on %s!"
            % (Opts.Server3Name, Opts.Server3VMname, Opts.VHostName),
        )
        Opts.ExitCode = 12
        sys.exit(Opts.ExitCode)
...
def RemoveCloneOnTarget(Opts, Num):
    ...
    Opts.Raise = False
    if Opts.VHostType == "vmware":
        CxCommand = "vim-cmd vmsvc/device.diskremove %s %s %s false" % (
            Opts.VMid,
            Opts.Controller[1],
            Num,
        )
        ...
        ErrMsg(
            Opts,
            "don't know how to remove cloned disk #%s (%s) from %s on %s!"
            % (Num, Opts.CloneDisk[Num], Opts.Server3Name, Opts.VHostName),
        )
        Opts.ExitCode = 13
        sys.exit(Opts.ExitCode)
    return True
...
def ListMappedVolumesForHost(Opts, HostName):
    ...
    Msg(
        Opts,
        "trying to list volumes from %s mapped to %s..."
        % (Opts.Storage1Name, HostName),
    )
    HostPrefix = "SR-"
    Opts.MappedVolume = []
    if Opts.Storage1Type == "storwize":
                            ...
                            if TextMappedScsiId and len(TextMappedScsiId) > 0:
                                try:
                                    ...
                                    if MappedScsiId >= Opts.ScsiOffset:
                                        ...
                                        Msg(
                                            Opts,
                                            "volumes found mapped %s"
                                            % (Opts.MappedVolume,),
                                        )
                                except TypeError:
                                    ErrorMsg(
                                        Opts,
                                        "problem interpreting the list of mapped volumes for %s at %s"
                                        % (HostName, Opts.Storage1Name),
                                    )
            return Opts.MappedVolume
        except:
            ErrMsg(
                Opts,
                "problem gettting the list of volumes mapped to %s%s"
                % (HostPrefix, HostName),
            )
            return []
    else:
        ErrMsg(
            Opts, "don't know how to list mapped volumes for %s" % (Opts.Storage1Type,)
        )
        return []
...
def ListSnapshotOnVHost(Opts, Num):
    ...
    Msg(
        Opts,
        "trying to get the list of snapshot volumes attached to VMware eSX hosts...",
    )
    if Opts.VHostType == "vmware":
        try:
            CxCommand = (
                "esxcli storage vmfs snapshot list | awk '/UUID/ {print $3}' | tee SNAPSHOT%sUUID.txt"
                % (Opts.LUN[Num],)
            )
            OutputText = UNIXconnect(Opts, Opts.VHostUser, Opts.VHostName, CxCommand)
            for OutputLine in OutputText.splitlines():
                if OutputLine and len(OutputLine) > 0:
                    ...
                    if len(SnapshotUUID) == 35:
                        ...
                        ErrMsg(
                            Opts,
                            "got too much for UUID (%s), exiting..." % (SnapshotUUID,),
                        )
                        Opts.ExitCode = 51
                        sys.exit(Opts.ExitCode)
                    else:
                        ErrMsg(
                            Opts,
                            "got too little for UUID (%s), exiting..."
                            % (SnapshotUUID,),
                        )
                        Opts.ExitCode = 25
                        sys.exit(Opts.ExitCode)
        ...
        ErrMsg(
            Opts, "don't know how to list the SAN snapshots on %s!" % (Opts.VHost1Name,)
        )
        Opts.ExitCode = 42
        sys.exit(Opts.ExitCode)
    return True
...
def ListMountedSnapshotFSOnVHost(Opts):
    ...
    Msg(
        Opts,
        "trying to get the list of mounted snapshot filesystems on VMware ESX hosts...",
    )
    if Opts.VHostType == "vmware":
        try:
...
def ResigSnapshotOnVHost(Opts, Num):
    ...
    if Opts.VHostType == "vmware":
        try:
            Msg(
                Opts,
                "resigning snapshot of %s using signature %s"
                % (
                    Opts.LUN[Num],
                    Opts.SnapshotUUID[Num],
                ),
            )
            CxCommand = "esxcli storage vmfs snapshot resignature -u %s" % (
                Opts.SnapshotUUID[Num],
            )
            OutputText = UNIXconnect(Opts, Opts.VHostUser, Opts.VHostName, CxCommand)
            MiniWait(Opts)
            ...
            CxCommand = (
                "esxcli storage filesystem list | awk '$0~/snap/ && $0~/%s/{print substr($0,72,35)}'"
                % (Opts.LUN[Num],)
            )
            OutputText = UNIXconnect(Opts, Opts.VHostUser, Opts.VHostName, CxCommand)
            if OutputText and len(OutputText) > 0:
            ...
            Msg(
                Opts,
                "unmounting snapshot filesystem with UUID %s"
                % (Opts.SnapshotUUID[Num],),
            )
            CxCommand = "esxcli storage filesystem unmount -u %s" % (
                Opts.SnapshotUUID[Num],
            )
            OutputText = UNIXconnect(Opts, Opts.VHostUser, Opts.VHostName, CxCommand)
        except subprocess.CalledProcessError as e:
            if e.returncode != 0:
                ErrMsg(
                    Opts,
                    "%s (%s) on %s returned %d!"
                    % (FnName, CxCommand, Opts.VHostName, e.returncode),
                )
                ErrMsg(Opts, "%s" % (OutputText,))
    else:
        ErrMsg(
            Opts,
            "don't know how to resignature the SAN snapshot on %s!" % (Opts.VHostName,),
        )
        Opts.ExitCode = 43
        sys.exit(Opts.ExitCode)
    return True
...
def MountSnapshotOnVHosts(Opts, Num):
    ...
    if Opts.VHostType == "vmware":
        try:
            Msg(
                Opts,
                "mounting (temporary) snapshot using signature %s"
                % (Opts.SnapshotUUID[Num],),
            )
            CxCommand = "esxcli storage filesystem mount -u %s" % (
                Opts.SnapshotUUID[Num],
            )
            OutputText = UNIXconnect(Opts, Opts.VHostUser, Opts.VHostName, CxCommand)
        except subprocess.CalledProcessError as e:
...
def ListSnapshotFSOnVHost(Opts, Num):
    ...
    if Opts.VHostType == "vmware":
        CxCommand = (
            "esxcli storage filesystem list | awk '$2~/snap-.*%s/ {print $1}' | tee SNAPSHOT%sFSNAME.txt"
            % (
                Opts.LUN[Num],
                Opts.LUN[Num],
            )
        ...
        try:
            ...
            if len(OutputLines) > 0 and len(OutputLines[0]) > 0:
                ...
                ErrMsg(
                    Opts,
                    "could not get snapshot file system name for %s! on %s!"
        ...
        # GET NAA
        CxCommand = (
            "esxcli storage vmfs extent list | awk '$1~/snap.*%s/ {print $4}' | tee SNAPSHOT%sNAA.txt"
            % (
                Opts.LUN[Num],
                Opts.LUN[Num],
            )
        ...
        try:
            ...
            if len(OutputLines) > 0 and len(OutputLines[0]) > 0:
                ...
                ErrMsg(
                    Opts,
                    "could not get snapshot NAA for %s! on %s!"
                    % (Opts.LUN[Num], Opts.VHostName),
                )
                return False
        except IndexError:
        ...
        ErrMsg(
            Opts,
            "don't know how to get the NAA the SAN snapshot on %s!" % (Opts.VHostName,),
        )
        Opts.ExitCode = 44
        sys.exit(Opts.ExitCode)
    return True
...
def GetDataStoreInFSOnVHost(Opts, Num):
    ...
    if Opts.VHostType == "vmware":
        CxCommand = "ls %s | tee SNAPSHOT%sDSNAME.txt" % (
            Opts.SnapshotFS[Num],
            Opts.LUN[Num],
        )
        OutputText = UNIXconnect(Opts, Opts.VHostUser, Opts.VHostName, CxCommand)
        ...
        if len(OutputLines) > 0 and len(OutputLines[0]) > 0:
            ...
            Msg(
                Opts,
                "found datastore [%s] on file system %s"
                % (Opts.SnapshotDS[Num], Opts.SnapshotFS[Num]),
            )
        else:
        ...
        ErrMsg(
            Opts,
            "don't know how to get the datastore of the SAN snapshot on %s!"
            % (Opts.VHostName,),
        )
        Opts.ExitCode = 45
        sys.exit(Opts.ExitCode)
...
def GetDataStoresOnVHost(Opts, FSName):
    ...
    if Opts.VHostType == "vmware":
        try:
            ...
            if OutputText and len(OutputText) > 0:
                for OutputLine in OutputText.splitlines():
                    if (
                        OutputLine
                        and len(OutputLine) > 0
                        and OutputLine.split()[0]
                        and len(OutputLine.split()[0]) > 0
                    ):
                        DatastoreName = OutputLine.split()[0]
                        Msg(
                            Opts,
                            "found datastore [%s] on filesystem '%s'"
                            % (DatastoreName, FSName),
                        )
        ...
        ErrMsg(
            Opts,
            "don't know how to list datastores for servers of type '%s'"
            % (Opts.VHostType,),
        )
        Opts.ExitCode = 46
        sys.exit(Opts.ExitCode)
        ...
        if len(Opts.VMDKlist) > 0:
            for GivenVMDK in Opts.VMDKlist:
                try:
                    ...
                    Opts.Raise = False
                    CxCommand = "ls -1 '%s'/%s-flat.vmdk 2>/dev/null" % (
                        FSName,
                        GivenVMDK,
                    )
                    OutputText = UNIXconnect(
                        Opts, Opts.VHostUser, Opts.VHostName, CxCommand
                    )
            ...
            Msg(
                Opts,
                "because the list of filtered VMDKs is empty, will search for everyone...",
            )
            try:
                ...
                ErrMsg(
                    Opts,
                    "problem searching for all VMDKs on filesystem '%s'" % (FSName,),
                )
    else:
        ErrMsg(
            Opts,
            "don't know how to search VMDKs for servers of type '%s'"
            % (Opts.VHostType,),
        )
        Opts.ExitCode = 47
        sys.exit(Opts.ExitCode)
...
def GetVMDKInFSOnVHost(Opts, Num):
    ...
    Msg(
        Opts,
        "trying to look for VMDK in %s/%s..."
        % (
            Opts.SnapshotFS[Num],
            Opts.SnapshotDS[Num],
        ),
    ...
    if Opts.VHostType == "vmware":
        CxCommand = (
            """ls %s/%s | awk '$NF!~/flat.vmdk/{print $NF}' | tee %sVMDKNAME.txt"""
            % (Opts.SnapshotFS[Num], Opts.SnapshotDS[Num], Opts.LUN[Num])
        )
        OutputText = UNIXconnect(Opts, Opts.VHostUser, Opts.VHostName, CxCommand)
        OutputLines = OutputText.splitlines()
        if len(OutputLines) > 0 and len(OutputLines[0]) > 0:
            ...
            Msg(
                Opts,
                "found virtual disk %s/%s/%s"
                % (Opts.SnapshotFS[Num], Opts.SnapshotDS[Num], Opts.SnapshotVMDK[Num]),
            )
        else:
        ...
        ErrMsg(
            Opts,
            "don't know how to get the VMDK of the SAN snapshot on %s!"
            % (Opts.VHostName,),
        )
        Opts.ExitCode = 46
        sys.exit(Opts.ExitCode)
    ...
    if Opts.VHostType == "vmware":
        if len(Opts.SnapshotVMDK) > 0:
            ...
            for FullVmdkName in Opts.SnapshotVMDK:
                try:
                    CxCommand = "vim-cmd vmsvc/device.diskaddexisting %s %s %s %s" % (
                        Opts.VMid,
                        FullVmdkName,
                        Opts.Controller[1],
                        VirtualDiskIndex,
                    ...
                    ErrMsg(
                        Opts,
                        "problem attaching '%s' like disk [%s:%s] to VM %s"
                        % (
                            FullVmdkName,
                            Opts.Controller[1],
                            VirtualDiskIndex,
        ...
        ErrMsg(
            Opts,
            "don't know how to attach virtual disks in hosts of type %s"
            % (Opts.VHostType,),
        )
...
def AttachCloneDiskToTarget(Opts, Num):
    ...
    Msg(
        Opts,
        "trying to attach the cloned VMDK (%s/%s/%s) to target VM..."
        % (
            Opts.SnapshotFS[Num],
            Opts.SnapshotDS[Num],
            Opts.SnapshotVMDK[Num],
        ),
    )
    if Opts.VHostType == "vmware":
        Msg(Opts, "trying to attach existing virtual disk %s" % (Num,))
        CxCommand = "vim-cmd vmsvc/device.diskaddexisting %s %s/%s/%s %s %s" % (
            Opts.VMid,
            Opts.SnapshotFS[Num],
            Opts.SnapshotDS[Num],
            Opts.SnapshotVMDK[Num],
        ...
        ErrMsg(
            Opts,
            "don't know how to attach 1st cloned disk from %s on %s!\n"
...
def StartTargetVM(Opts):
    ...
    if Opts.VHostType == "vmware":
        ...
        ErrMsg(
            Opts,
            "don't know how to start %s (%s) on %s!"
            % (Opts.Server3Name, Opts.Server3VMname, Opts.VHostName),
        )
        Opts.ExitCode = 49
        sys.exit(Opts.ExitCode)
    ...
    try:
        for i in range(len(Opts.CloneTimes)):
            ...
            if (len(CleanedTime) > 0) and (CleanedTime not in Opts.ExecutionTimes):
                ...
                ErrMsg(
                    Opts,
                    'could not interpret time "%s", skipping...\n'
                    % (Opts.CloneTimes[i],),
                )
        ...
        try:
            ...
            if OutputText and len(OutputText) > 0:
                for OutputLine in OutputText.splitlines():
                    if (
                        OutputLine.split()[1]
                        and len(OutputLine.split()[1]) > 0
                        and OutputLine.split()[1] == VolName
                    ):
                        VolumeExists = True
                        break
                if not VolumeExists:
                    ErrMsg(
                        Opts,
                        'given volume "%s" was not found!... removing from LUN list...'
                        % (VolName,),
                    )
            ...
            ErrMsg(
                Opts,
                "could not validate volumes on %s... exiting" % (Opts.Storage1Name,),
            )
            sys.exit(5)
    else:
        ErrMsg(
            Opts,
            "don't know how to check volumes on %s... exiting" % (Opts.Storage1Name,),
        )
        sys.exit(4)
...
def ValidateGivenVolumes(Opts):
    ...
    if len(Opts.LUN) > 0:
        LunList = list(
            Opts.LUN
        )  # Use a copy of the LUN list because removing items breaks the loop
...
def CreateNowNamedVolume(Opts, VolumeName):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            ...
            CxCommand = (
                "mkvdisk -mdiskgrp %s -iogrp %d -size %d -unit b -rsize %d%% -warning %d%% -autoexpand -easytier %s -name %s"
                % (
                    Opts.Pool,
                    Opts.IOgroup,
                    NewSnapSize,
            ...
            if len(OutputText) > 0:
                for OneLine in OutputText.splitlines():
                    if OneLine.startswith("vdisk_UID:"):
                        ...
                        Msg(
                            Opts,
                            "new volume %s VUID is %s" % (NowVolumeName, NewVolumeVUID),
                        )
...
def CloneVolumeToVolume(Opts, SourceVolume, TargetVolume):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            MatchOb = re.match(
                "%s_c_([0-9]{8}_[0-9]{6})" % (SourceVolume), TargetVolume
            )
            if MatchOb:
            ...
            CxCommand = "mkfcmap -source %s -target %s -name %s -copyrate %d" % (
                SourceVolume,
                TargetVolume,
                CloneNow,
                Opts.SnapCopy,
            ...
            Msg(
                Opts,
                "snapshot clone of volume %s created into %s (pair named %s)"
                % (SourceVolume, TargetVolume, CloneNow),
            )
        except:
            ErrMsg(
                Opts,
                "snapshot clone of volume %s could not created into %s!"
...
def IsESXlongUUID(FSstring):
    try:
        LongMatch = re.match(
            "^([0-9a-f]{8}-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{12})$", FSstring
        )
...
def IsMounted(Opts, VHU, VHN, FSname):
    ...
    if IsESXfsPath(FSname):
        ...
        CxCommand = "esxcli storage filesystem list | awk -v F='%s' 'NR>2 && $1==F'" % (
            FSname,
        )
    elif IsESXshortUUID(FSname):
        Msg(Opts, "%s is a VMware short UUID, checking..." % (FSname,))
        CxCommand = (
            "esxcli storage filesystem list | awk -v U='%s' 'NR>2 && $(NF-5)==U'"
            % (FSname,)
        )
    ...
    if Opts.Storage1Type == "storwize":
        try:
            if IsMounted(Opts, Opts.VHost1User, Opts.VHost1Name, SnapshotFSname):
                Msg(
                    Opts,
                    "unmounting snapshot fs with path %s from %s"
                    % (
                        SnapshotFSname,
                        Opts.VHost1Name,
                    ),
                )
                CxCommand = "esxcli storage filesystem unmount -p %s" % (
                    SnapshotFSname,
                )
                OutputText = UNIXconnect(
                    Opts, Opts.VHost1User, Opts.VHost1Name, CxCommand
                )
            ...
            ErrMsg(
                Opts,
                "error unmounting fs with path %s from VMware hosts!\n"
                % (SnapshotFSname,),
            )
            sys.exit(6)
    return True
...
def RemoveSANAttachment(Opts, SnapshotNAA):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            Msg(
                Opts,
                "detaching from LUN with NAA %s on %s"
                % (
                    SnapshotNAA,
                    Opts.VHost1Name,
                ),
            )
            CxCommand = "esxcli storage core device set --state=off -d %s" % (
                SnapshotNAA,
            )
            OutputText = UNIXconnect(Opts, Opts.VHost1User, Opts.VHost1Name, CxCommand)
            Msg(
...
def ExitIfKillFile(Opts):
    ...
    if os.path.exists(Opts.KillFile):
        ...
        try:
            ...
            sys.stdout.write(
                "%s: kill file found and removed, exiting...\n" % (Opts.PrgName,)
            )
            FileLog(Opts, "kill file found and removed, exiting...")
        except IOError:
            sys.stdout.write(
                "%s: error trying to remove killfile (%s)\n"
...
def MapVolumeToVHosts(Opts, VolumeName):
    ...
    if Opts.Storage1Type == "storwize":
        try:
            ...
            Msg(
                Opts,
                "mapping to SR-%s [SCSI ID %d]" % (Opts.VHost1Name, Opts.ScsiOffset),
            )
            CxCommand = "mkvdiskhostmap -scsi %d -host %s -force %s" % (
                Opts.ScsiOffset,
                "SR-" + Opts.VHost1Name,
                VolumeName,
            )
            ...
            Msg(
                Opts,
                "mapping to SR-%s [SCSI ID %d]" % (Opts.VHost2Name, Opts.ScsiOffset),
            )
            CxCommand = "mkvdiskhostmap -scsi %d -host %s -force %s" % (
                Opts.ScsiOffset,
                "SR-" + Opts.VHost2Name,
...
def GetNAAForVHostFilesystem(Opts, FSName):
    ...
    if Opts.VHostType == "vmware":
        try:
            ...
            UUID = FSName.split("/")[3]
            CxCommand = (
                "esxcli storage vmfs extent list | awk -v UUID=%s '$2==UUID{print $4; exit}' | tee SNAPSHOTNAA.%s.txt"
                % (UUID, UUID)
            )
            OutputText = UNIXconnect(Opts, Opts.VHostUser, Opts.VHostName, CxCommand)
            for OutputLine in OutputText.splitlines():
        ...
        ErrMsg(
            Opts, 'don\'t know get the NAA of the volume of "%s", exiting..' % (FSName,)
        )
        Opts.ExitCode = 12
        sys.exit(Opts.ExitCode)
    return False
    ...
    if Opts.VHostType == "vmware":
        for ESXhost in [Opts.VHost1Name, Opts.VHost2Name]:
            try:  # SR-HOST1
                ...
                OutputText = UNIXconnect(
                    Opts, Opts.VHost1User, ESXhost, CxCommand
                )  # Assumes VHost1User==VHost2User
                for OutputLine in OutputText.splitlines():
                    if OutputLine and len(OutputLine) > 0:
                        NAA = OutputLine.split()[0]
                        if len(NAA) == 36:
...
def RemoveClonesFromTarget(Opts):
    ...
    for i in range(
        Options.ScsiOffset
    ):  # range(x) goes from 0 to x-1, change to number of disks
        RemoveCloneOnTarget(
            Options, Options.ScsiOffset + i
        )  # de-attach VMDK from Windows VM
    Opts.Verbose = SaveVerbose
    return True
...
def FlatList(AnyList):
    ...
    for ItemElement in AnyList:
        if (
            ItemElement != ItemElement.split(",")[0]
        ):  # It item was broken, then is a composed element
...
try:
    ...
    parser.add_option(
        ...
        action="store_false",
        default=True,
        help="Quiet mode. Don't show messages",
    )
    parser.add_option(
        "-o",
        "--output",
        ...
        action="store",
        type="string",
        help="Log execution into file name",
        default="%s.log" % (PrgName,),
    )
    parser.add_option(
        "-O",
        "--no-log",
    ...
    parser.add_option(
        ...
        "--ur",
        "--remote-unix-command",
        dest="RemoteUnixCommand",
        action="store",
        type="string",
        help="Remote execution command for Unix connections",
        default="plink",
    )
    parser.add_option(
        "-R",
        ...
        dest="RemoteWindowsCommand",
        action="store",
        type="string",
        help="Remote execution command for Windows connections",
        default="psexec",
    )
    parser.add_option(
        "--s1n",
    ...
    parser.add_option(
        ...
        action="store",
        type="string",
        help="Server #1 username to connect to",
        default="sqlservice",
    )
    parser.add_option(
        "--s1t",
    ...
    parser.add_option(
        ...
        action="store",
        type="string",
        help="Server #2 username to connect to",
        default="sqlservice",
    )
    parser.add_option(
        "--s2t",
    ...
    parser.add_option(
        ...
        action="store",
        type="string",
        help="Server #3 username to connect to",
        default="sqlservice",
    )
    parser.add_option(
        "--s3t",
    ...
    parser.add_option(
        ...
        action="store",
        type="string",
        help="Storage server #1 hostname",
        default="V3700",
    )
    parser.add_option(
        "--t1u",
        ...
        action="store",
        type="string",
        help="Storage server #1 username to connect to",
        default="soporte",
    )
    parser.add_option(
        "--t1t",
        ...
        action="store",
        type="string",
        help="Storage server #1 type of OS",
        default="storwize",
    )
    parser.add_option(
        "--v1n",
        ...
        action="store",
        type="string",
        help="Virtualization host #1 hostname",
        default="ESX11CO",
    )
    parser.add_option(
        "--v1u",
        ...
        action="store",
        type="string",
        help="Virtualization host #1 username to connect to",
        default="root",
    )
    parser.add_option(
        "--v1t",
        ...
        action="store",
        type="string",
        help="Virtualization host #1 type of OS",
        default="vmware",
    )
    parser.add_option(
        "--v2n",
        ...
        action="store",
        type="string",
        help="Virtualization host #2 hostname",
        default="ESX06CO",
    )
    parser.add_option(
        "--v2u",
        ...
        action="store",
        type="string",
        help="Virtualization host #2 username to connect to",
        default="root",
    )
    parser.add_option(
        "--v2t",
        ...
        action="store",
        type="string",
        help="Virtualization host #2 type of OS",
        default="vmware",
    )
    parser.add_option(
        "-c",
        ...
        action="store",
        type="int",
        help="How many snapshots clones to maintain",
        default=2,
    )
    parser.add_option(
        "-p",
        ...
        action="store",
        type="int",
        help="How thin make snapshots, percent integer",
        default=5,
    )
    parser.add_option(
        "-P",
    ...
    parser.add_option(
        ...
        action="store",
        type="int",
        help="Time between snapshots (seconds)",
        default=60 * 60,
    )
    parser.add_option(
        "-w",
    ...
    parser.add_option(
        ...
        action="append",
        type="string",
        help="VMDK file names to attach",
    )
    parser.add_option(
        "-K",
        "--kill",
        ...
        action="store_true",
        default=False,
        help="Kill another instance of script",
    )
    parser.add_option(
        "--run-each",
        dest="RunOClock",
    ...
    if len(Args) > 0:
        ...
        if len(Args) != len(Options.LUN):
            ...
            ErrMsg(
                Options,
                "Not all given volumes exist on SAN storage or not all were usable",
            )
    ...
    while True:
        ...
        for OneVolume in Options.LUN:
            LastCloneSnapshot = GetNameOfLastNamedSnap(
                Options, OneVolume
            )  # Get name of las clone of VolumeName
            Msg(
                Options,
                "found %s as last clone snapshot for %s"
                % (LastCloneSnapshot, OneVolume),
            )
            ...
            GetDataStoresOnVHost(
                Options, Options.SnapshotFS[-1]
            )  # Datastores on last snapshot filesystem
            GetVMDKsOnVHost(
                Options, Options.SnapshotFS[-1]
            )  # All VMDKs on last snapshot filesystem
