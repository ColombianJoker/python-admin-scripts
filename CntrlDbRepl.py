...
def WINconnect(Opts, ServerName, CommandString):
    ...
    try:
        ...
        if not Opts.DEBUG:
            ...
            sys.exit(
                Opts.ExitCode
            )  # Exit code 9 now defined as ERROR in PSEXEC CONNECTION
    return CxLines
...
def MessageFn(Opts, M):
    if Opts.Verbose:
        if M == 1:
            ...
            sys.stdout.write(
                "%s: getting %s virtual machine state...\n"
                % (Opts.PrgName, Opts.Server3VMname)
            )
        elif M == 4:
            sys.stdout.write(
                "%s: shutting down target VM (%s)...\n"
                % (Opts.PrgName, Opts.Server3VMname)
            )
        elif M == 5:
            sys.stdout.write(
                "%s: powering off target VM (%s)...\n"
                % (Opts.PrgName, Opts.Server3VMname)
            )
        elif M == 6:
            sys.stdout.write(
                "%s: removing clone disk on target (%s)...\n"
                % (Opts.PrgName, Opts.Server3VMname)
            )
        elif M == 7:
            sys.stdout.write(
                "%s: unmapping clone disk on target (%s)...\n"
                % (Opts.PrgName, Opts.Server3VMname)
            )
        elif M == 8:
            sys.stdout.write(
                "%s: stopping services on target (%s)...\n"
                % (Opts.PrgName, Opts.Server3VMname)
            )
        elif M == 9:
            sys.stdout.write(
                "%s: stopping MS SQL Server Agent on target (%s)...\n"
                % (Opts.PrgName, Opts.Server3VMname)
            )
        elif M == 10:
            sys.stdout.write(
                "%s: stopping MS SQL Server on target (%s)...\n"
                % (Opts.PrgName, Opts.Server3VMname)
            )
        elif M == 11:
            sys.stdout.write(
                "%s: flushing first disk cache on target (%s)...\n"
                % (Opts.PrgName, Opts.Server3VMname)
            )
        elif M == 12:
            sys.stdout.write(
                "%s: stopping MS SQL Server first source (%s)...\n"
                % (Opts.PrgName, Opts.Server1Name)
            )
        elif M == 13:
            sys.stdout.write(
                "%s: stopping MS SQL Server Agent first source (%s)...\n"
                % (Opts.PrgName, Opts.Server1Name)
            )
        elif M == 14:
            sys.stdout.write(
            ...
            sys.stdout.write(
                "%s: flushing disk cache on first source (%s)...\n"
                % (Opts.PrgName, Opts.Server1Name)
            )
        elif M == 16:
            sys.stdout.write(
                "%s: replicating volume on storage %s...\n"
                % (Opts.PrgName, Opts.Storage1Name)
            )
        elif M == 17:
            sys.stdout.write(
                "%s: starting services on first source (%s)...\n"
                % (Opts.PrgName, Opts.Server1Name)
            )
        elif M == 18:
            sys.stdout.write(
                "%s: starting MS SQL Server on first source (%s)...\n"
                % (Opts.PrgName, Opts.Server1Name)
            )
        elif M == 19:
            sys.stdout.write(
                "%s: starting MS SQL Server Agent on first source (%s)...\n"
                % (Opts.PrgName, Opts.Server1Name)
            )
        elif M == 20:
            sys.stdout.write(
                "%s: stopping services on second source (%s)...\n"
                % (Opts.PrgName, Opts.Server2Name)
            )
        elif M == 21:
            sys.stdout.write(
                "%s: stopping MS SQL Server Agent on second source (%s)...\n"
                % (Opts.PrgName, Opts.Server2Name)
            )
        elif M == 22:
            sys.stdout.write(
                "%s: stopping MS SQL Server on second source (%s)...\n"
                % (Opts.PrgName, Opts.Server2Name)
            )
        elif M == 23:
            sys.stdout.write(
                "%s: flushing disk cache on second source (%s)...\n"
                % (Opts.PrgName, Opts.Server2Name)
            )
        elif M == 24:
            sys.stdout.write(
                "%s: starting services on second source (%s)...\n"
                % (Opts.PrgName, Opts.Server2Name)
            )
        elif M == 25:
            sys.stdout.write(
                "%s: starting MS SQL Server on second source (%s)...\n"
                % (Opts.PrgName, Opts.Server2Name)
            )
        elif M == 26:
            sys.stdout.write(
                "%s: starting MS SQL Server Agent on second source (%s)...\n"
                % (Opts.PrgName, Opts.Server2Name)
            )
        elif M == 27:
            sys.stdout.write(
                "%s: re-scanning SAN from all adapters on virtualization host (%s)...\n"
                % (Opts.PrgName, Opts.VHost1Name)
            )
        elif M == 28:
            sys.stdout.write(
                "%s: searching for snaphosts on virtualization host (%s)...\n"
                % (Opts.PrgName, Opts.VHost1Name)
            )
        elif M == 29:
            sys.stdout.write(
                "%s: resigning snapshot on virtualization host (%s)...\n"
                % (Opts.PrgName, Opts.VHost1Name)
            )
        elif M == 30:
            sys.stdout.write(
                "%s: searching for new snapshot file system on virtualization host (%s)...\n"
                % (Opts.PrgName, Opts.VHost1Name)
            )
        elif M == 31:
            sys.stdout.write(
                "%s: searching for new datastores on virtualization host (%s)...\n"
                % (Opts.PrgName, Opts.VHost1Name)
            )
        elif M == 32:
            sys.stdout.write(
                "%s: searching for new virtual disk files on virtualization host (%s)...\n"
                % (Opts.PrgName, Opts.VHost1Name)
            )
        elif M == 33:
            sys.stdout.write(
                "%s: trying to attach clone disk to target (%s)...\n"
                % (Opts.PrgName, Opts.Server3Name)
            )
        elif M == 34:
            sys.stdout.write(
                "%s: starting virtual machine (%s)...\n"
                % (Opts.PrgName, Opts.Server3VMname)
            )
        elif M == 35:
            sys.stdout.write(
                "%s: reassigning a drive letter on target (%s)...\n"
                % (Opts.PrgName, Opts.Server3Name)
            )
        elif M == 36:
            sys.stdout.write(
                "%s: searching for mounted NFS file systems on virtualization host (%s)...\n"
                % (Opts.PrgName, Opts.VHost1Name)
            )
        elif M == 37:
            sys.stdout.write(
                "%s: adding and mounting NFS file systems on virtualization host (%s)...\n"
                % (Opts.PrgName, Opts.VHost1Name)
            )
        elif M == 38:
            sys.stdout.write(
                "%s: unmounting and removing NFS file systems on virtualization host (%s)...\n"
                % (Opts.PrgName, Opts.VHost1Name)
            )
        elif M == 39:
            sys.stdout.write(
                "%s: trying to attach virtual disk to target (%s)...\n"
                % (Opts.PrgName, Opts.Server3Name)
            )
        elif M == 40:
            sys.stdout.write(
                "%s: trying to attach NFS VMDK disk to target (%s)...\n"
                % (Opts.PrgName, Opts.Server3Name)
            )
        sys.stdout.flush()
...
def GetVirtualMachine(Opts, VMname):
    ...
    if Opts.VHostType == "vmware":
        ...
        if len(Opts.VMid) == 0:
            sys.stderr.write(
                "%s: could not get the VM ID of %s, exiting...\n"
                % (Opts.PrgName, Opts.Server3Name)
            )
            Opts.ExitCode = 50
        ...
        sys.stderr.write(
            "%s: don't know how to shutdown %s (%s) on %s!\n"
...
def GetVirtualMachineState(Opts):
    ...
    if Opts.VHostType == "vmware":
        ...
        if len(Opts.VMid) == 0:
            sys.stderr.write(
                "%s: could not get the state of VM %s (%s) on %s, exiting...\n"
...
def PowerOffTarget(Opts):
    ...
    if Opts.VHostType == "vmware":
        ...
        sys.stderr.write(
            "%s: don't know how to stop %s (%s) on %s!\n"
            % (Opts.PrgName, Opts.Server3Name, Opts.Server3VMname, Opts.VHostName)
        )
        Opts.ExitCode = 12
        sys.exit(Opts.ExitCode)
...
def RemoveCloneOnTarget(Opts, Num):
    ...
    MessageFn(Opts, 6)
    MessageSpecific(
        Opts, "trying to remove disk %s (%s)..." % (Num, Opts.CloneDisk[Num])
    )
    Opts.Raise = False
    if Opts.VHostType == "vmware":
        ...
        sys.stderr.write(
            "%s: don't know how to remove cloned disk #%s (%s) from %s on %s!\n"
...
def UnmapCloneLun(Opts, Num):
    ...
    MessageFn(Opts, 7)
    MessageSpecific(
        Opts, "trying to unmap disk %s (%s)..." % (Num, Opts.CloneDisk[Num])
    )
    if Opts.Storage1Type == "storwize":
        MessageSpecific(Opts, "unmapping from %s..." % ("SR-" + Opts.VHost2Name,))
        CxCommand = "rmvdiskhostmap -host %s %s_snapshot1" % (
            "SR-" + Opts.VHost2Name,
            Opts.LUN[Num],
        )
        ...
        sys.stderr.write(
            "%s: don't know how to un-map volumes on %s!\n"
            % (Opts.PrgName, Opts.Storage1Name)
        )
        Opts.ExitCode = 21
        sys.exit(Opts.ExitCode)
...
def StopSQLServerOnTarget(Opts):
    ...
    if Opts.Server3Type == "windows":
        ...
        sys.stderr.write(
            "%s: dont' know how to connect and stop MS SQL Server on %s!\n"
            % (Opts.PrgName, Opts.Server3Name)
        )
        Opts.ExitCode = 31
        sys.exit(Opts.ExitCode)
...
def Flush1stCacheOnTarget(Opts):
    ...
    if Opts.Server3Type == "windows":
        ...
        sys.stderr.write(
            "%s: dont' know how to flush cache on %s!\n"
            % (Opts.PrgName, Opts.Server3Name)
        )
        Opts.ExitCode = 32
        sys.exit(Opts.ExitCode)
...
def FlushCacheOnSource1(Opts):
    ...
    if Opts.Server1Type == "windows":
        try:
            ...
            if e.returncode == 6:
                sys.stderr.write(
                    "•• %s: security problem connecting to %s!\n"
                    % (Opts.PrgName, Opts.Server1Name)
                )
    else:
        sys.stderr.write(
...
def ReplicateVolumeOnStorage(Opts, Num):
    ...
    if Opts.Storage1Type == "storwize":
        MiniWait(Opts)
        MessageSpecific(
            Opts, "stopping snapshot pair %s_snapshot_pair1" % (Opts.LUN[Num],)
        )
        CxCommand = "stopfcmap %s_snapshot_pair1" % (Opts.LUN[Num],)
        OutputText = UNIXconnect(Opts, Opts.Storage1User, Opts.Storage1Name, CxCommand)
        MiniWait(Opts)
        MessageSpecific(
            Opts, "preparing snapshot pair %s_snapshot_pair1" % (Opts.LUN[Num],)
        )
        CxCommand = "prestartfcmap -restore %s_snapshot_pair1" % (Opts.LUN[Num],)
        OutputText = UNIXconnect(Opts, Opts.Storage1User, Opts.Storage1Name, CxCommand)
        MiniWait(Opts)
        MessageSpecific(
            Opts, "starting snapshot pair %s_snapshot_pair1" % (Opts.LUN[Num],)
        )
        CxCommand = "startfcmap -restore %s_snapshot_pair1" % (Opts.LUN[Num],)
        OutputText = UNIXconnect(Opts, Opts.Storage1User, Opts.Storage1Name, CxCommand)
        MiniWait(Opts)
        MessageSpecific(
            Opts, "mapping to %s [SCSI ID %s]" % ("SR-" + Opts.VHost2Name, 10 + Num)
        )
        CxCommand = "mkvdiskhostmap -scsi %s -host %s -force %s_snapshot1" % (
            10 + Num,
            "SR-" + Opts.VHost2Name,
            Opts.LUN[Num],
        )
        ...
        MiniWait(Opts)
        MessageSpecific(
            Opts, "mapping to %s [SCSI ID %s]" % ("SR-" + Opts.VHost1Name, 10 + Num)
        )
        CxCommand = "mkvdiskhostmap -scsi %s -host %s -force %s_snapshot1" % (
            10 + Num,
            "SR-" + Opts.VHost1Name,
        ...
        sys.stderr.write(
            "%s: don't know how to replicate volumes on %s!\n"
            % (Opts.PrgName, Opts.Storage1Name)
        )
        Opts.ExitCode = 23
        sys.exit(Opts.ExitCode)
...
def StartSQLServerOnSource1(Opts):
    ...
    if Opts.Server1Type == "windows":
        ...
        sys.stderr.write(
            "%s: dont' know how to connect and start MS SQL Server on %s!\n"
            % (Opts.PrgName, Opts.Server1Name)
        )
        Opts.ExitCode = 35
        sys.exit(Opts.ExitCode)
...
def RescanSANonVHost(Opts):
    ...
    if Opts.VHost1Type == "vmware":
        ...
        sys.stderr.write(
            "%s: don't know how to rescan the SAN on %s!\n"
            % (Opts.PrgName, Opts.VHost1Name)
        )
        Opts.ExitCode = 41
        sys.exit(Opts.ExitCode)
...
def ListSnapshotOnVHost(Opts, Num):
    ...
    if Opts.VHostType == "vmware":
        ...
        MessageSpecific(
            Opts, "got UUID %s for %s" % (Opts.SnapshotUUID[Num], Opts.CloneDisk[Num])
        )
        if len(Opts.SnapshotUUID[Num]) < 35:
            sys.stderr.write(
                "%s: %s could not get %s snapshot UUID! (got %s)\n%s: Exiting...\n"
                % (
                    Opts.PrgName,
                    FnName,
                    Opts.CloneDisk[Num],
        ...
        if len(Opts.SnapshotUUID[Num]) > 35:
            sys.stderr.write(
                "%s: %s got too much for %s snapshot UUID! (got %s)\n%s: Exiting...\n"
                % (
                    Opts.PrgName,
                    FnName,
                    Opts.CloneDisk[Num],
        ...
        sys.stderr.write(
            "%s: don't know how to list the SAN snapshots on %s!\n"
            % (Opts.PrgName, Opts.VHost1Name)
        )
        Opts.ExitCode = 42
        sys.exit(Opts.ExitCode)
...
def ResigSnapshotOnVHost(Opts, Num):
    ...
    if Opts.VHostType == "vmware":
        try:
            MessageSpecific(
                Opts,
                "resigning snapshot using signature %s" % (Opts.SnapshotUUID[Num],),
            )
...
def ListSnapshotFSOnVHost(Opts, Num):
    ...
    if Opts.VHostType == "vmware":
        CxCommand = (
            "esxcli storage filesystem list | awk '/snap.*%s/ {print $1}' | tee SNAPSHOT%sFSNAME.txt"
            % (
                Opts.LUN[Num],
                Opts.LUN[Num],
            )
        ...
        if len(OutputLines[0]) > 0:
            ...
            sys.stderr.write(
                "%s: could not get snapshot file system name for %s! on %s!\n"
        ...
        sys.stderr.write(
            "%s: don't know how to get the filesystem of the SAN snapshot on %s!\n"
            % (Opts.PrgName, Opts.VHostName)
        )
        Opts.ExitCode = 44
        sys.exit(Opts.ExitCode)
...
def GetDataStoreInFSOnVHost(Opts, Num):
    ...
    if Opts.VHostType == "vmware":
        ...
        if len(OutputText) > 0:
            ...
            sys.stderr.write(
                "%s: could not get datastore name for %s!\n"
        ...
        sys.stderr.write(
            "%s: don't know how to get the datastore of the SAN snapshot on %s!\n"
            % (Opts.PrgName, Opts.VHostName)
        )
        Opts.ExitCode = 45
        sys.exit(Opts.ExitCode)
...
def GetVMDKInFSOnVHost(Opts, Num):
    ...
    if Opts.VHostType == "vmware":
        ...
        sys.stderr.write(
            "%s: don't know how to get the VMDK of the SAN snapshot on %s!\n"
            % (Opts.PrgName, Opts.VHostName)
        )
        Opts.ExitCode = 46
        sys.exit(Opts.ExitCode)
...
def AttachCloneDiskToTarget(Opts, Num):
    ...
    if Opts.VHost1Type == "vmware":
        ...
        sys.stderr.write(
            "%s: don't know how to attach 1st cloned disk from %s on %s!\n"
...
def StartTargetVM(Opts):
    ...
    if Opts.VHostType == "vmware":
        ...
        sys.stderr.write(
            "%s: don't know how to start %s (%s) on %s!\n"
            % (Opts.PrgName, Opts.Server3Name, Opts.Server3VMname, Opts.VHostName)
        )
        Opts.ExitCode = 49
        sys.exit(Opts.ExitCode)
...
def ReassignLetterOnTarget(Opts):
    ...
    if Opts.Server3Type == "windows":
        ...
        sys.stderr.write(
            "%s: dont' know how to change letters on %s!\n"
            % (Opts.PrgName, Opts.Server3Name)
        )
        Opts.ExitCode = 32
        sys.exit(Opts.ExitCode)
...
def NFSmounted(Opts):
    ...
    if Opts.VHost1Type == "vmware":
        ...
        CxCommand = (
            "esxcli storage nfs list | grep -q '^%s' && echo True || echo False"
            % (Options.NFSvol)
        )
        OutputText = UNIXconnect(Opts, Opts.VHost1User, Opts.VHost1Name, CxCommand)
        if len(OutputText) > 0:
            ...
            sys.stderr.write(
                "%s: could not list mounted NFS filesystems on %s!\n"
                % (Opts.PrgName, Opts.VHost1Name)
            )
            Opts.ExitCode = 64
            sys.exit(Opts.ExitCode)
        ...
        sys.stderr.write(
            "%s: don't know how to list mounts on %s!\n"
            % (Opts.PrgName, Opts.VHost1Name)
        )
        Opts.ExitCode = 49
        sys.exit(Opts.ExitCode)
...
def NFSmount(Opts):
    ...
    if Opts.VHost1Type == "vmware":
        MessageSpecific(
            Opts, "adding %s:%s like %s" % (Opts.NFS1Name, Opts.NFS1Export, Opts.NFSvol)
        )
        CxCommand = "esxcli storage nfs add --host %s --share %s --volume-name %s" % (
            Opts.NFS1Name,
            Opts.NFS1Export,
            Opts.NFSvol,
        )
...
def NFSumount(Opts):
    ...
    if Opts.VHost1Type == "vmware":
        ...
        sys.stderr.write(
            "%s: don't know how to umount %s:%s (%s)!\n"
...
def AttachNFSVMDK(Opts, FSDSpath, Num):
    ...
    if Opts.VHost1Type == "vmware":
        ...
        CxCommand = "ls -1 %s | awk '/\.vmdk$/ && $NF!~/flat.vmdk/{print $NF}'" % (
            FSDSpath,
        )
        OutputText = UNIXconnect(Opts, Opts.VHost1User, Opts.VHost1Name, CxCommand)
        if len(OutputText[0]) > 0:
            ...
            MessageSpecific(
                Opts,
                "VMDK found (%s), trying to attach to target VM (%s)"
                % (NFSVMDK, Opts.Server3VMname),
            )
            CxCommand = "vim-cmd vmsvc/device.diskaddexisting %s %s/%s %s %s" % (
                Opts.VMid,
                FSDSpath,
                NFSVMDK,
                Opts.Controller[1],
            ...
            sys.stderr.write(
                "%s: could not get snapshot file system name for %s!\n"
                % (Opts.PrgName, Opts.CloneDisk[Num])
            )
            Opts.ExitCode = 62
            sys.exit(Opts.ExitCode)
        ...
        if NFSmounted(Opts):
            ...
            if len(OutputText) > 0:
                ...
                for DS in Opts.NFSDS[Num:]:
                    ...
                    AttachNFSVMDK(
                        Opts, VMDKfullPath, LocalDisksOffset + SANDisksOffset + Num
                    )
    else:
        sys.stderr.write(
            "%s: don't know how to attach NFS virtual disks on %s!\n"
            % (Opts.PrgName, Opts.VHost1Name)
        )
        Opts.ExitCode = 49
        sys.exit(Opts.ExitCode)
...
try:
    ...
    parser.add_option(
        ...
        action="store",
        type="string",
        help="NFS server #1 hostname",
        default="192.168.20.146",
    )
    parser.add_option(
        "--n1e",
        "--nfs1export",
        ...
        action="store",
        type="string",
        help="NFS server #1 export",
        default="/export/db1-clone",
    )
    parser.add_option(
        "-w",
        "--shutdown-wait",
